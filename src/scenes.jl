...
function final_scene()::scene
    ...
    push!(world.objects, sphere(SA_F64[0.0, -1000.0, 0.0], 1000.0, ground_material, 1000.0^2))
    for a in -11:1:11
        for b in -11:1:11
    ...
    push!(world.objects, sphere(SA_F64[0.0, 1.0, 0.0], 1.0, material1, 1.0^2.0))

    material2 = lambertian(color(SA_F32[0.4, 0.2, 0.1]))
    push!(world.objects, sphere(SA_F64[-4.0, 1.0, 0.0], 1.0, material2, 1.0^2.0))

    material3 = metal(color(SA_F32[0.7, 0.6, 0.5]), 0.0)
    push!(world.objects, sphere(SA_F64[4.0, 1.0, 0.0], 1.0, material3, 1.0^2.0))
...
function basic_scene()::scene
    ...
    push!(world.objects, sphere(SA_F64[0.0, -100.5, -1.0], 100.0, ground_materal, 100.0^2))
    push!(world.objects, sphere(SA_F64[0.0, 0.0, -1.0], 0.5, center_material, 0.5^2))
    push!(world.objects, sphere(SA_F64[-1.0, 0.0, -1.0], 0.5, left_material, 0.5^2))
    push!(world.objects, sphere(SA_F64[-1.0, 0.0, -1.0], -0.45, left_material, (-0.45)^2))
    push!(world.objects, sphere(SA_F64[1.0, 0.0, -1.0], 0.5, right_material, 0.5^2))
